...
当前项目进度与运行命令见 [README.md](./README.md)。用户提供的 `tesla_style_fronted` 配色已用于桌面工作台；左侧显示猎聘或 BOSS 企业端页面，右侧直接显示 DSH Agent 对话。Agent 可从自然语言保存岗位、识别和读取当前页面（包括搜索结果）、操作搜索框、进入推荐页、打开候选人资料详情；动作显示彩色边框与移动指针。DSH 插件也能逐项分析简历并提交带岗位指纹与简历原文校验的草稿。分析队列支持按岗位条件快照筛选、去重和人工复核，但尚无招聘网站的稳定候选人 ID。项目固定依赖 DSH `0.1.5-rc.2`，运行时使用 npm 发布包与 AgentHR 专用 preset。真实招聘页面仍由用户自行验证。

**当前交付点：可由招聘人员手动验证的对话式 MVP，真实网站验收尚未进行。** 左侧为招聘浏览器，右侧为内嵌 DSH 对话；岗位表单和分析记录在次级页。Agent 可在未配置岗位时读取任意已加载的站内页面和搜索结果，按页面快照操作搜索框、搜索按钮与滚动；配置岗位后可进入推荐页、打开资料详情、判断技能证据并保存待复核草稿。浏览器动作展示渐变阴影边框与会停留在上次位置的移动指针。可按用户要求汇总页面可见的薪资和意向；不主动联系候选人。
...
当前实现选择 Electron 自己持有的 [`WebContents` / `WebFrameMain`](https://www.electronjs.org/docs/latest/api/web-frame-main)：主进程读取有上限的页面快照，DSH 通过本机受控工具拿到页面文本和临时元素引用；搜索动作重新校验引用后，在同一个浏览器视图里输入或点击。回车使用 Electron 的 [`sendInputEvent`](https://www.electronjs.org/docs/latest/api/web-contents)；离线 Electron 假搜索页已验证 `rawKeyDown → char → keyUp` 能触发表单提交。此方案已具备 browser-use 的“观察 → 决策 → 动作 → 再观察”循环，不要求 DSH 进程直接附着 CDP。若真实站点出现复杂跨进程 iframe、Canvas 控件或需要截图定位，再把**同一主进程边界**的动作执行层升级为 CDP；DSH 插件调用接口可以不变。DSH 的实验性 Inspector 面向 DSH 自身调试，不能直接替代招聘浏览器的操作适配器。
...
当前插件暴露读取/保存岗位、读取浏览器状态、读取当前页快照、操作搜索/导航控件、进入推荐页、读取当前页卡片、打开指定卡片详情、读取已打开简历、保存分析草稿十个**业务工具**。页面快照对搜索结果页同样可用，动作必须引用最新快照中的元素；打开详情前也重新比对当前卡片内容。页面内显示操作轨迹；不提供任意脚本、读取 Cookie 或发送消息。未来若加入对话与发送工具，发送前需要校验平台、账号、候选人、会话最新状态和幂等键。

## 4. 核心招聘流程

以下是长期完整流程。**第一版只实现受控的资料详情打开、只读分析与技能确认问题草稿**；可读取页面可见的薪资和求职意向。实际沟通与主动追问留到后续阶段。
...
| 自然语言分析闭环 | DSH 十个工具、自然语言岗位配置与分析草稿已实现；真实资料效果待用户验证 | 核对 tMCAO 未写明时的“相关线索/未知”、原文证据、技能问题草稿和分析队列 |
| 可见网页动作 | 进入推荐页、按卡片指纹打开资料详情、边框与指针提示已实现；真实站点待用户验证 | 核对两站卡片、详情入口和视觉反馈；动作不能触发沟通 |
| 页面适配修正 | 待真实页面反馈 | 按失败步骤修正猎聘/BOSS 选择器、iframe、弹窗和正文提取 |
| 站内搜索与结果读取 | 通用页面快照和搜索框操作已实现；真实站点待用户验证 | 检查 Java 等关键词输入、提交、结果页可见文本；按反馈补专用筛选和结构化结果适配 |
...
> 请读取当前岗位和我已经打开的简历。逐项判断岗位技能要求，引用简历原文；如果只写了“动物实验”而没明确写 tMCAO，不要判定为已掌握或明确不符。生成简短的技能确认问题草稿，并保存待人工复核的分析卡片。不要发送消息。

随后在工作台刷新分析队列，核对来源平台、证据、问题草稿和人工状态。完整的操作顺序见 [README 手动验证](./README.md#招聘人员手动验证)。

## 7. 第一版范围

当前手动验证版处理真实招聘数据，因此采用**受控页面动作与只读分析**：**Agent 站内搜索或招聘人员手动筛选 → Agent 读取搜索结果与候选人资料 → 按岗位条件判断 tMCAO 证据 → 生成技能确认问题草稿 → 保存带证据的分析卡片**。搜索与浏览不要求先配置岗位；评估时才要求岗位标准。用户可以要求汇总页面可见的薪资或求职意向；Agent 不自动联系候选人，也不把未知技能写成已确认。小核酸、ASO、CNS 等岗位方向共用同一套“岗位条件 + 证据 + 问题草稿”机制。

当前原型允许 Agent 读取搜索结果页文本，操作快照中识别出的搜索框与搜索/导航按钮；复杂筛选和搜索结果结构化卡片仍待实际页面适配。BOSS 详情关闭后可能残留 iframe，因此读取前后均检查推荐页与简历 iframe 唯一可见，无法确定时停止读取。真实页面的有效性仍以招聘人员的手动验证为准。
