...
**当前交付点：可由招聘人员手动验证的对话式 MVP，真实网站验收尚未进行。** 左侧为招聘浏览器，右侧为内嵌 DSH 对话；岗位表单和分析记录在次级页。Agent 可在未配置岗位时读取任意已加载的站内页面和搜索结果，按页面快照操作搜索框、搜索按钮与滚动；配置岗位后可进入推荐页、打开资料详情、判断技能证据并保存待复核草稿。浏览器动作展示彩色边框与移动指针。Agent 不发送消息，也不询问薪资或求职意向。
...
当前手动验证版处理真实招聘数据，因此采用**受控页面动作与只读分析**：**Agent 站内搜索或招聘人员手动筛选 → Agent 读取搜索结果与候选人资料 → 按岗位条件判断 tMCAO 证据 → 生成技能确认问题草稿 → 保存带证据的分析卡片**。搜索与浏览不要求先配置岗位；评估时才要求岗位标准。不自动联系候选人，不追问薪资或求职意向，也不把未知技能写成已确认。小核酸、ASO、CNS 等岗位方向共用同一套“岗位条件 + 证据 + 问题草稿”机制。
