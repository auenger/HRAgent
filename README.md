...
独立 Electron 招聘工作台。左侧是内嵌招聘网站，右侧直接显示 DeepSeek Harness（DSH）Agent 对话；“岗位与记录”保留为次级页。Agent 可根据自然语言保存岗位、识别当前页面、读取搜索结果、操作站内搜索框、进入推荐页、打开指定候选人详情并保存有证据的分析草稿。浏览器动作显示彩色边框和移动指针。两站适配器都尚未经真实招聘页面验证。
...
- `src/main/bridge.ts`：仅监听 `127.0.0.1` 的随机端口，要求启动时生成的 Bearer Token；提供页面状态、搜索结果等当前页快照、候选人卡片、简历和岗位条件，并允许操作快照中的搜索控件、保存岗位、进入推荐页、按卡片指纹打开详情、提交分析草稿。不开放任意页面脚本执行。
...
- `src/plugin/index.ts`：向 DSH 注册 10 个受控业务工具：读取/保存岗位、识别页面、读取任意已加载站内页快照、操作搜索/导航控件、进入推荐页、读取卡片、打开详情、读取简历、保存分析草稿。搜索和读页面无需先配置岗位；评估候选人时才需要岗位条件。不存在消息发送工具。
...
第一版现在可以让 Agent 在当前站内页面识别搜索框、填入关键词、提交搜索，并读取搜索结果页的可见文本；搜索框和按钮由页面快照定位，网页变化后须重新读取快照。复杂筛选项和搜索结果卡片结构尚未做跨站稳定适配，用户仍可手动操作。GoodHR5 的平台选择器只是参考，真实页面效果由招聘人员验证。
...
3. 核对 Agent 是否在左侧搜索框输入、提交搜索并读取搜索结果；再请它打开其中一人的详情。核对红色边框与移动指针，以及是否只打开资料、不触发沟通。也可手动打开简历。
...
本地检查只使用模拟 DOM、桥接层模拟请求、模拟分析数据、插件工具到桥接和分析库的完整模拟链路、类型检查、构建，以及隔离临时目录里的 DSH Host 和 App GUI。`test` 还会创建真实 DSH Agent 会话并检查工具目录：AgentHR preset 只能看到 10 个招聘工具，没有 Shell、文件或通用 Web 工具。未登录或操作真实招聘账号。猎聘选择器及 DSH 工具的实际页面运行效果需要由用户在 App 中验证。DSH 会话本身可能记录 Agent 读到的简历内容；当前“不保存完整简历正文”仅指 AgentHR 的分析 SQLite 库。
