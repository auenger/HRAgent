...
const server = createServer((req, res) => {
  ...
  if (req.url === '/search') res.end(`<!doctype html><style>
    .action {cursor:pointer;padding:10px;display:inline-block} #cities[hidden],#cities [hidden],#detail[hidden]{display:none}
    ...
    <p id="results">尚未搜索</p><a id="candidate" href="javascript:;">周** · 教育 SaaS 销售</a>
...
try {
  ...
  const act = async (label, action = 'click', value) => {
    ...
    return cdp.act({ snapshotId: snapshot.snapshotId, ref: control.ref, action, value })
  }
  ...
  assert.ok(snapshot.frames.some(frame => frame.accessibility.some(node => node.name === '人才关键词')))
  await act('城市（北京）')
  await act('福建')
  await act('福州')
  await act('人才关键词', 'fill', '教育信息化')
  await read()
  ...
  try {
    ...
    const browserAct = async (label, action = 'click', value) => {
      ...
      await browser.actOnPage({ snapshotId: page.snapshotId, ref: control.ref, action, value })
    ...
    await browserAct('福建')
    await browserAct('福州')
    await browserAct('人才关键词', 'fill', '教育 SaaS')
    await browserAct('搜索')
    ...
    await browserAct('关闭')
    await assert.rejects(browser.readOpenResume(), /可见简历/)
